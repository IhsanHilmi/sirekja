...
class ApprovalLine extends Component
{
    ...
    public $dept_head_id = "",  $bisnis_unit_id = "",$hr_unit_id = "", $direksi_1_id = "", $direksi_2_id = "", $direksi_3_id = "", $presdir_id = "", $corp_hr_id = "", $superadmin_id = "";
    ...
    protected $rules = [
        ...
        'direksi_2_id' => ['required', 'exists:users,id'],
        'direksi_3_id' => ['required', 'exists:users,id'],
    ...
    protected function assignEmptyStringIfNull($field) {
        if(trim($field) === ""){
            return null;
        }
            return $field;
    ...
    public function openFormModalForEdit(ModelsApprovalLine $ALModel) {
        ...
        $this->hr_unit_id = optional($ALModel->users()->wherePivot('approves_as','HR Unit')->first())->id ?? "";
    ...
    public function saveData() {
        $this->validate();
        $this->dept_head_id = $this->assignEmptyStringIfNull($this->dept_head_id);
        $this->hr_unit_id = $this->assignEmptyStringIfNull($this->hr_unit_id);
        $this->direksi_1_id = $this->assignEmptyStringIfNull($this->direksi_1_id);
        $this->direksi_2_id = $this->assignEmptyStringIfNull($this->direksi_2_id);
        $this->direksi_3_id = $this->assignEmptyStringIfNull($this->direksi_3_id);
        $this->presdir_id = $this->assignEmptyStringIfNull($this->presdir_id);
        $this->corp_hr_id = $this->assignEmptyStringIfNull($this->corp_hr_id);
        $this->superadmin_id = $this->assignEmptyStringIfNull($this->superadmin_id);
        ...
        if(isset($this->cursorId)){
            ...
            $alus = [
                ['approval_line_id' => $al->id, 'user_id' => $this->hr_unit_id,'approves_as' => 'HR Unit', 'order' =>1],
                ['approval_line_id' => $al->id, 'user_id' => $this->dept_head_id, 'approves_as' => 'Dept. Head', 'order' => 2],
                ['approval_line_id' => $al->id, 'user_id' => $this->direksi_1_id,'approves_as' => 'Direksi 1', 'order' =>3],
                ['approval_line_id' => $al->id, 'user_id' => $this->direksi_2_id,'approves_as' => 'Direksi 2', 'order' =>4],
                ['approval_line_id' => $al->id, 'user_id' => $this->direksi_3_id,'approves_as' => 'Direksi 3', 'order' =>5],
                ['approval_line_id' => $al->id, 'user_id' => $this->presdir_id,'approves_as' => 'Presdir', 'order' => 6],
                ['approval_line_id' => $al->id, 'user_id' => $this->corp_hr_id,'approves_as' => 'Corp. HR', 'order' => 7],
                ['approval_line_id' => $al->id, 'user_id' => $this->superadmin_id,'approves_as' => 'Superadmin', 'order' => 8]
            ];
            foreach ($alus as $alu) {
